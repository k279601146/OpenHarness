...
class ImagegenCliTool(BaseTool):
    ...
    async def execute(self, arguments: ImagegenCliInput, context: ToolExecutionContext) -> ToolResult:
        ...
        try:
            ...
            return await _media_billing_error_result(context, f"imagegen CLI timed out after {arguments.timeout_seconds}s.\n{output}".strip())

        output = (completed.stdout or "").strip()
        if completed.returncode != 0:
            return await _media_billing_error_result(context, output or f"imagegen CLI failed with code {completed.returncode}")
