    ...
    *,
    ...
    for directory in directories:
        ...
        for path in candidates:
            ...
            reported_path = normalize_path_func(path) if normalize_path_func else str(path)
            reported_base_dir = normalize_path_func(path.parent) if normalize_path_func else str(path.parent)
            skill_type = str(frontmatter.get("skill-type") or frontmatter.get("skill_type") or "logic")
            skills.append(
                SkillDefinition(
                    name=name,
...
def _skill_aliases(frontmatter: dict[str, object]) -> tuple[str, ...]:
    aliases: list[str] = []
