...
_RESERVED_BUNDLED_SKILL_NAMES = frozenset(
    {
...
def _iter_bundled_skill_files(target_dir: Path) -> list[Path]:
    ...
    direct_files = sorted(
        path for path in target_dir.glob("*.md") if _bundled_command_name(path) not in _RESERVED_BUNDLED_SKILL_NAMES
    )
    directory_files = sorted(
        skill_file
        for child in target_dir.iterdir()
        if child.is_dir()
        and child.name not in _RESERVED_BUNDLED_SKILL_NAMES
        and (skill_file := child / "SKILL.md").exists()
    )
    return sorted(
        [*direct_files, *directory_files],
        key=_bundled_command_name,
    )


def _bundled_command_name(path: Path) -> str:
    return path.stem if path.name != "SKILL.md" else path.parent.name
