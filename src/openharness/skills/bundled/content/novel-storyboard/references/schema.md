...
`promptLang` 可省略（**默认 `en`——官方规范口径**）：整条英文、禁角色名，台词在 `<d>[Chinese]` 里保留原文。设成 `zh` 可切整条中文（对齐指令、字段名、镜头标记都有中文版，人名放行）——偏离官方推荐的备选项。`style` 可省略（默认 `realistic`），也可使用首页 `style-catalog.json` 的 94 个模板 ID 或 `custom`；每个模板直接提供独立英文短语，必须与角色/场景 skill 使用同一个 ID，对应短语必须出现在**每条**分镜图提示词里——同一部剧的分镜图不许画风漂，门查。
