...
| --- | --- | --- |
| `source` | 是 | 剧名/书名 |
| `style` | 是 | 使用首页 `style-catalog.json` 中的模板 ID，或 `realistic` / `ghibli` / `custom`；每个模板拥有独立环境提示词，与 novel-characters 同名对齐 |
| `scenes` | 是 | 场景数组 |
...
7. 风格与反向词匹配：每个注册模板必须使用其预设渲染句和反向词；`sheet` 必须含当前预设渲染句
