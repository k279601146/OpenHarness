...
这些 prompt recipes 用于 OpenHarness SaaS 的 `imagegen_cli` 图片生成和编辑流程。
...
The labeled lines are prompt scaffolding, not a closed schema. `Asset type` and `Input images` are prompt-only scaffolding; they are not dedicated provider payload fields.

Execution details such as `quality`, `input_fidelity`, masks, output formats, and local output paths are `imagegen_cli`/provider parameters. For transparent images, prompt for a flat chroma-key background and remove it locally with `python "${CODEX_HOME:-$HOME/.codex}/skills/.system/imagegen/scripts/remove_chroma_key.py"` when the returned file is readable. Use model-native transparency only after the user confirms a switch to a supported model.

Model notes:
- `gpt-image-2` is the default GPT Image model for new workflows.
- `gpt-image-2` supports `quality` values `low`, `medium`, `high`, and `auto`.
- For 4K-style `gpt-image-2` output, use `3840x2160` or `2160x3840`.
- If transparent output needs native transparency, ask before using `gpt-image-1.5` unless the user already explicitly requested it. Explain that chroma-key removal is the default path, but `gpt-image-2` does not support `background=transparent`.
...
Post-process note: after `imagegen_cli` generation, run `python "${CODEX_HOME:-$HOME/.codex}/skills/.system/imagegen/scripts/remove_chroma_key.py" --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill` when the returned file is readable. Ask before using `gpt-image-1.5` with `background=transparent` for true/native transparency, failed chroma-key validation, or complex subjects such as hair, glass, smoke, liquids, translucent materials, reflections, or soft shadows, unless the user already explicitly requested that model.
